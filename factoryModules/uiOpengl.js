...
var uiOpengl = (function() {
  	...
  	API.factoryView = function( options )
	{
		...
		var ogl = require('Ti.OpenGL');
		
		var glView = ogl.createView({
			backgroundColor:"#000",
			top:10,
			left:10,
			width:300,
		...
		var gestureListener = new GestureListener(glView);
		...
		var setup = function(view) {
			...
			view.glMatrixMode(ogl.GL_PROJECTION);
			view.glLoadIdentity();
			view.glFrustumf(-1.0, 1.0, -h, h, 5.0, 200.0);
			view.glMatrixMode(ogl.GL_MODELVIEW);
	
			view.glEnable(ogl.GL_CULL_FACE);
			view.glEnable(ogl.GL_LIGHTING);
			view.glEnable(ogl.GL_LIGHT0);
			view.glEnable(ogl.GL_DEPTH_TEST);
			view.glShadeModel(ogl.GL_SMOOTH);
			view.glLightfv(ogl.GL_LIGHT0, ogl.GL_POSITION, pos);
			var light0Ambient = [0.1, 0.1, 0.1, 1.0];
			view.glLightfv(ogl.GL_LIGHT0, ogl.GL_AMBIENT, light0Ambient);
			var light0Diffuse = [2.0, 2.0, 2.0, 1.0];
			view.glLightfv(ogl.GL_LIGHT0, ogl.GL_DIFFUSE, light0Diffuse);
			var light0Specular = [1.5, 1.5, 1.5, 1.0];
			var light0Shininess = 0.4;
			view.glLightfv(ogl.GL_LIGHT0, ogl.GL_SPECULAR, light0Specular);
			var light0Direction = [0.0, 0.0, -1.0];
			view.glLightfv(ogl.GL_LIGHT0, ogl.GL_SPOT_DIRECTION, light0Direction);
			view.glLightf(ogl.GL_LIGHT0, ogl.GL_SPOT_CUTOFF, 100.0);
			
			view.glEnableClientState(ogl.GL_NORMAL_ARRAY);
			view.glEnableClientState(ogl.GL_VERTEX_ARRAY);
			view.glEnable(ogl.GL_NORMALIZE);
	
			view.glFinish();
			
			gear1 = new GearMaker(1.0, 4.0, 1.0, 20, 0.7, red, ogl);
			gear2 = new GearMaker(0.5, 2.0, 2.0, 10, 0.7, green, ogl);
			gear3 = new GearMaker(1.3, 2.0, 0.5, 10, 0.7, blue, ogl);
		}//end setup
		...
		var draw_gear = function(view, gear) {
			Ti.API.info( 'draw gear');
			view.glMaterialfv(ogl.GL_FRONT_AND_BACK, ogl.GL_AMBIENT_AND_DIFFUSE, gear.color);
			view.glVertexPointer(3, ogl.GL_FLOAT, 0, gear.vertexBuffer);
			view.glNormalPointer(ogl.GL_FLOAT, 0, gear.normalBuffer);
			view.glDrawElements(ogl.GL_TRIANGLES, gear.nindices/3, ogl.GL_UNSIGNED_SHORT, gear.indexBuffer);
		}//end draw_gear
		
		//config the ogl render
		setup(glView);
		
		//config the drawFrame callback
		var drawFrame = function(e) {
			drawImage(glView, new Date().getTime());
		}
		var interval = 33;
	...
	{
		var win = Ti.UI.createWindow({title:'opengl'});
...
//Ti.UI.currentWindow.add( uiOpengl.factoryView({}) );
//uiOpengl.factoryWindow({}).open({modal:true});
uiOpengl.factoryWindow({}).open({fullscreen:true});
