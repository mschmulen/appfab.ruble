var uiTable = (function() {
	...
	API.customObject = {
  	...
  	API.factoryView = function(opts){ 
	    ...
	    button.addEventListener('click', function() {
	    	...
	    	Cloud.Objects.create(API.customObject, function (e) {
	            if (e.success) { alert('Created!'); }
	            else { error(e); }
	        });
    ...
    topView.add(buttonRefresh);
   	...
   	buttonRefresh.addEventListener('click', function() {
        ...
        Cloud.Objects.query({
        	classname: API.customObject.classname
		}, function (e) {
		    if (e.success) {
		   		var objects = e[API.customObject.classname];
		   		if (!objects.length) { alert('No objects found!'); }
                else
                {
			        API.data = [];
                	for (var i = 0; i < objects.length; i++) {
			            var obj = objects[i];
			           	...
			           	API.data.push({title: obj.id, hasChild:true, obj:obj }) ;
			        }
			        tv.setData( API.data );
                }//end else there are oobjects
		    } else {
	...
	tv.addEventListener('click', function(e)
	{
		...
		//walk all the e.rowData.obj.created_at,
